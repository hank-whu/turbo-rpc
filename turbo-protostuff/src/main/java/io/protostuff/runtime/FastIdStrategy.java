...
public final class FastIdStrategy extends IdStrategy {
	...
	public FastIdStrategy(IdStrategy primaryGroup, int groupId) {
		super(DEFAULT_FLAGS, primaryGroup, groupId);
	}

	public FastIdStrategy(int flags, IdStrategy primaryGroup, int groupId) {
		super(flags, primaryGroup, groupId);
	}

	/**
	 * Registers a pojo. Returns true if registration is successful or if the same
	 * exact schema was previously registered. Used by
	...
	/**
	 * Registers a pojo. Returns true if registration is successful or if the same
	 * exact schema was previously registered.
	 */
	...
	public <T extends Enum<T>> boolean registerEnum(Class<T> enumClass) {
		return null == enumMapping.putIfAbsent(enumClass.getName(), EnumIO.newEnumIO(enumClass, this));
	}
	...
	static Class<?> resolveClass(String className) {
		final RuntimeFieldFactory<Object> inline = RuntimeFieldFactory.getInline(className);
